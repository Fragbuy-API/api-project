...
async def lookup_barcode(lookup: BarcodeLookup):
    """
    Lookup a barcode in the database and return associated SKU and product name if found.
    """
    logger.info(f"Barcode lookup request received for: {lookup.barcode}")
    
    try:
        # Query the barcode in the barcodes table and join with products for name
        query = text("""
            SELECT b.sku, b.alternate, p.description as name 
            FROM barcodes b
        ...
        logger.info(f"Barcode found: {lookup.barcode}, SKU: {sku}, Name: {name}")
        return {
            "status": "success",
